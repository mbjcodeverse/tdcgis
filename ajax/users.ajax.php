...
class AjaxUsers{
	...
	public function ajaxActivateUser(){
		...
		$answer = (new UsersModel)->mdlUpdateUser($table, $item1, $value1, $item2, $value2);
	}
	...
	public function ajaxValidateUser(){
		$item = "user";						// field name from table users
		$value = $this->validateUser;		// text input from user box
		$answer = (new ControllerUsers)->ctrShowUsers($item, $value);
		echo json_encode($answer);
	}
}
